...
# Run directly from the repo root, e.g. `torchrun tools/eval_baselines/eval_blip.py`: put the repo root on
...
from src.data.collator.eval_collator import BLIP2Collator
from src.evaluation.eval_utils import get_pred, print_results, save_results
...
def get_pred_blip(qry_t, tgt_t, mode="multimodal2text"):
    ...
    if mode == "multimodal2text":
        ...
        scores = np.dot(
            qry_t, tgt_t.T
        )  # (32, dim) dot (candidate_num, dim).T -> (32, candidate_num)

        # Find the maximum score for each candidate across the 32 tokens
        max_scores = np.max(
            scores, axis=0
        )  # Max along the 32 tokens for each candidate (shape candidate_num)
        ...
        scores = np.dot(
            tgt_t, qry_t
        )  # (candidate_num, 32, dim) dot (dim) -> (candidate_num, 32)
