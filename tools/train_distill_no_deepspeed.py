# Run directly from the repo root, e.g. `torchrun tools/train_distill_no_deepspeed.py`: put the repo root on
...
def finetune(
    ...
    num_trainable_params = sum(
        p.numel() for p in distiller.student.parameters() if p.requires_grad
    )
    num_total_params = sum(p.numel() for p in distiller.student.parameters())
    # None when --teacher_embedding_cache is in use: the teacher is never loaded.
    teacher_params = (
        list(distiller.teacher.parameters()) if distiller.teacher is not None else []
    )
    num_trainable_params_teacher = sum(
        p.numel() for p in teacher_params if p.requires_grad
    )
    num_total_params_teacher = sum(p.numel() for p in teacher_params)
    print_rank(
        f"Number of trainable parameters: {num_trainable_params} / {num_total_params}"
    )
    print_rank(
        f"Number of trainable parameters (teacher): {num_trainable_params_teacher} / {num_total_params_teacher}"
    )
            ...
            if accelerator.sync_gradients:
                if (
                    ...
                    accelerator.clip_grad_norm_(
                        distiller.student.parameters(), training_args.max_grad_norm
                    )
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
