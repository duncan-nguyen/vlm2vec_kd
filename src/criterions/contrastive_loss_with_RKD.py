...
class ContrastiveLossWithRKD(nn.Module):
    ...
    def forward(self, distiller, input_data):
        ...
        student_qry_reps, _, _, output_hidden_states = student_model.encode_input(
            student_input_qry
        )
        student_pos_reps, _, _, output_hidden_states = student_model.encode_input(
            student_input_pos
        )

        # Via the distiller so this criterion also works against a precomputed
        # teacher embedding cache; RKD reads nothing but the final embedding.
        ...
        contrastive_loss = nn.CrossEntropyLoss()(
            scores / self.distiller.temperature, target
        )
