...
class ModelArguments:
    ...
    model_type: str = field(
        default=None,
        metadata={
            "help": "model type, typically includes in config file, but sometimes needs mannually add"
        },
    )
    ...
    num_crops: int = field(
        default=16, metadata={"help": "number of crops used in image encoder"}
    )
    uigraph_use: bool = field(
        default=False, metadata={"help": "Enable ui graph for token selection"}
    )
    uigraph_diff: int = field(
        default=1,
        metadata={
            "help": "Pixel difference used for constructing ui graph for token selection"
        },
    )
    ...
    uimask_ratio: float = field(
        default=0.5,
        metadata={
            "help": "Specify the percentage of patch tokens to skip per component for token selection"
        },
    )
    ...
    lm_skip_layer: str = field(
        default="[1,28,0]",
        metadata={
            "help": "Specify the layers of the language model to skip for token selection"
        },
    )
    ...
    init_lora_model: bool = field(
        default=False, metadata={"help": "initializing with lora model"}
    )
    # distiller args:
    teacher_backbone: str = field(
        default=None, metadata={"help": "teacher model backbone"}
    )
    teacher_model_name: str = field(
        default=None, metadata={"help": "teacher model name or path"}
    )
    teacher_lora: bool = field(
        default=False, metadata={"help": "whether teacher is lora"}
    )
    ...
    teacher_lora_dropout: float = field(
        default=0.1, metadata={"help": "teacher lora dropout"}
    )
    ...
    teacher_pooling: str = field(
        default="last", metadata={"help": "pooling method for teacher encoder"}
    )
    ...
    projector_config_path: str = field(
        default=None,
        metadata={"help": "projector config path, if None, no projector will be used"},
    )
    projector_path: str = field(
        default=None,
        metadata={"help": "projector model path, if None, no projector will be used"},
    )
    projector_lr: float = field(
        default=1e-4, metadata={"help": "projector learning rate"}
    )
    student_hidden_dim: int = field(
        default=896, metadata={"help": "student hidden dim"}
    )
    teacher_hidden_dim: int = field(
        default=1536, metadata={"help": "teacher hidden dim"}
    )
    teacher_embedding_cache: str = field(
        default=None,
        metadata={
            "help": "directory of precomputed teacher embeddings (see tools/precompute_teacher_embeddings.py). When set, the teacher model is not loaded at all and its forward passes are replaced by a lookup; only criteria that read the final teacher embedding can use it"
        },
    )
    load_pretrained_lora: bool = field(
        default=False, metadata={"help": "load pretrained lora model for student"}
    )
...
class DataArguments:
    ...
    tgt_prefix_mod: bool = field(
        default=False, metadata={"help": "Modify the pos_prefix"}
    )
    chunk_size: int = field(
        default=32, metadata={"help": "Cluster sizes in metis. Only used in odibn"}
    )
    #!new args 2
    eval_dataset_name: str = field(
        default=None, metadata={"help": "Useful for datasets with subsets"}
    )
    eval_subset_name: list[str] = field(
        default=None, metadata={"help": "Useful for datasets with subsets"}
    )
    eval_image_dir: str = field(
        default=None, metadata={"help": "Eval Image directory path"}
    )
...
class TrainingArguments(TrainingArguments):
    ...
    gc_q_chunk_size: int = field(
        default=128, metadata={"help": "query side subset size. Should be power of 2"}
    )
    gc_p_chunk_size: int = field(
        default=128, metadata={"help": "target side subset size. Should be power of 2"}
    )
    interleave_stopping_strategy: str = field(
        default="all_exhausted", metadata={"help": "all_exhausted or first_exhausted"}
    )
    interleave_batch_size: float = field(
        default=0,
        metadata={
            "help": "Specify mini-batch size to interleave data from multi-sources, 0/None means random sampling by examples, 1 means full batch."
        },
    )
    #!new args
    gc_dynamic_limit: int = field(
        default=125,
        metadata={
            "help": "gc_chunk default limit - (128, 125) sized matrices works for Qwen2b. gc_dynamic_limit would be 125 and gc_p|q_chunk_size would be 128"
        },
    )
    #!new kd loss weight
    kd_weight: float = field(
        default=0.01, metadata={"help": "weight of kd loss in total loss"}
    )
    rkd_distance_weight: float = field(
        default=1.0, metadata={"help": "weight of distance loss in total kd loss"}
    )
    rkd_angle_weight: float = field(
        default=2.0, metadata={"help": "weight of angle loss in total kd loss"}
    )
    kd_loss_type: str = field(
        default="contrastive_rkd",
        metadata={"help": "type of kd loss, current only support RKD"},
    )
    ds_config: str = field(
        default=None, metadata={"help": "DeepSpeed config json file path"}
    )
    ...
    teacher_layer_mapping: list[int] = field(
        ...
        metadata={
            "help": "List of teacher layers used for distillation; number of elements equals number of projectors"
        },
    )
    student_layer_mapping: list[int] = field(
        default_factory=list,
        metadata={
            "help": "List of student layers used for distillation; number of elements equals number of projectors"
        },
    )
    split_layer_mapping: list[int] = field(
        default_factory=list,
        metadata={
            "help": "List of split layers for student; number of elements equals number of projectors"
        },
    )
