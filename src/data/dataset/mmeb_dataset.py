...
# The named presets deliberately do NOT agree with the training-side ones in
# src/data/images.py ("low" is 128 here and 448 there), so evaluating a
# checkpoint with a preset silently changes the preprocessing relative to how it
# was trained. Pass an explicit pixel budget instead -- e.g. --image_resolution
# 448 -- which gets the same "only shrink" semantics training uses.
_EVAL_PRESETS = {"high": 1344, "mid": 672, "low": 128}


def eval_image_target(resolution, max_dim=1344):
    """Longest side this image will end up at, whatever branch handles it.

    Used as the decode hint so the JPEG decoder does not produce pixels that
    are about to be thrown away; see `src.data.images.decode_image`.
    """
    if resolution in _EVAL_PRESETS:
        return _EVAL_PRESETS[resolution]
    try:
...
def process_image(image, resolution, max_dim=1344):
    """Resize an image for the encoder. See the note on the presets above."""
    if image is None:
        return None
    if resolution in _EVAL_PRESETS:
        # The presets resize unconditionally, including upwards.
        side = _EVAL_PRESETS[resolution]
        return image.resize((side, side))
    target_max = eval_image_target(resolution, max_dim)
    if max(image.size) > target_max:
        image = image.resize((target_max, target_max))
    return image


def open_eval_image(full_img_path, resolution):
    """Open an image already scaled down as far as the decoder can manage."""
    return (
        Image.open(full_img_path)
        if resolution is None
        else _drafted(full_img_path, eval_image_target(resolution))
    )


def _drafted(full_img_path, target_max):
    image = Image.open(full_img_path)
    # No-op for formats that cannot do it; for JPEG it picks a DCT scale that is
    # still at or above the requested size, so process_image below produces the
    # same geometry it always did.
...
def get_image_bytes_and_path(img_path, image_dir, model_backbone, image_resolution):
    ...
    resizes = backbone != PHI3V and image_resolution
    image = open_eval_image(full_img_path, image_resolution if resizes else None)
    if resizes:
...
def data_prepare(example, *args, **kwargs):
    ...
    neg_text_list = (
        [] if ((not neg_text_list) or type(neg_text_list) == str) else neg_text_list
    )
    neg_image_path_list = (
        []
        if ((not neg_image_path_list) or type(neg_image_path_list) == str)
        else neg_image_path_list
    )
    ...
    if model_backbone != PHI3V:
        qry_text = qry_text.replace(
            VLM_IMAGE_TOKENS[PHI3V], VLM_IMAGE_TOKENS[model_backbone]
        )
        pos_text = pos_text.replace(
            VLM_IMAGE_TOKENS[PHI3V], VLM_IMAGE_TOKENS[model_backbone]
        )
        neg_text_list = [
            neg_text.replace(VLM_IMAGE_TOKENS[PHI3V], VLM_IMAGE_TOKENS[model_backbone])
            if neg_text
            else ""
    ...
    qry_image = {
        "bytes": [None],
        "paths": [os.path.join(image_dir, qry_image_path) if qry_image_path else ""],
        "resolutions": [RESOLUTION_MAPPING.get(image_resolution, None)],
    }
    pos_image = {
        "bytes": [None],
        "paths": [os.path.join(image_dir, pos_image_path) if pos_image_path else ""],
        "resolutions": [RESOLUTION_MAPPING.get(image_resolution, None)],
    }
    # import ipdb; ipdb.set_trace()
    try:
        neg_image_path_list = [
            {
                "bytes": [None],
                "paths": [
                    os.path.join(image_dir, neg_image_path) if neg_image_path else ""
                ],
                "resolutions": [RESOLUTION_MAPPING.get(image_resolution, None)],
            }
            for neg_image_path in neg_image_path_list
        ]
    except:
        import ipdb
    ...
    return {
        ...
        "pos_text": pos_text_list,
        "pos_image": pos_image_list,
        "neg_text": [],
        "neg_image": [],
    }
...
def load_mmeb_dataset(model_args, data_args, training_args, *args, **kwargs):
    ...
    num_sample_per_subset = kwargs.get(
        "num_sample_per_subset", getattr(data_args, "num_sample_per_subset", None)
    )
    # dataset = load_dataset(dataset_name, subset_name, split=f"{dataset_split}")

    parquet_path = os.path.join(
        ".", dataset_name, subset_name, "train-00000-of-00001.parquet"
    )
    ...
    dataset = dataset.map(
        lambda x, idx: {**data_prepare(x, **kwargs), "idx": idx},
        with_indices=True,
        batched=False,
    ...
    print_master(
        f"Loaded {DATASET_PARSER_NAME}/{subset_name} dataset with {num_rows} samples"
    )
...
class TrainTextImageDataset(Dataset):
    def __init__(self, data_args, model_args):
        ...
        train_data = []
        print_rank(
            f"Loading {len(data_args.subset_name)} datasets: {data_args.subset_name}"
        )
    ...
    def _get_image(self, img_path):
        ...
        if backbone != PHI3V and self.data_args.image_resolution:
            return process_image(
                open_eval_image(full_img_path, self.data_args.image_resolution),
                self.data_args.image_resolution,
            )
        return Image.open(full_img_path)

    def __getitem__(self, data_idx) -> tuple[str, list[str]]:
        # A `print(..., flush=True)` per item used to live here. On a dataset of
        # this size that is millions of unbuffered writes competing with the
        # dataloader workers for the GIL and the terminal.
        qry_texts, qry_image_paths, pos_texts, pos_image_paths = (
            self.train_data[data_idx]["qry"],
            self.train_data[data_idx]["qry_image_path"],
            self.train_data[data_idx]["pos_text"],
            self.train_data[data_idx]["pos_image_path"],
        )
        if "neg_text" in self.train_data.column_names:
            neg_texts, neg_image_paths = (
                self.train_data[data_idx]["neg_text"],
                self.train_data[data_idx]["neg_image_path"],
            )
        else:
...
class EvalDataset(Dataset):
    ...
    def _get_image(self, img_path):
        ...
        full_img_path = os.path.join(self.data_args.image_dir, img_path)
        resizes = (
            self.model_args.model_backbone != PHI3V and self.data_args.image_resolution
        )
        image = open_eval_image(
            full_img_path, self.data_args.image_resolution if resizes else None
        )
        if resizes:
            image = process_image(image, self.data_args.image_resolution)
            image.filename = full_img_path  # to avoid error in datasets library
    ...
    def get_paired_data(self, text_field, img_path_field):
        ...
        for row in self.eval_data:
            if isinstance(row[text_field], str):
                ...
                assert type(row[img_path_field]) == list and len(
                    row[img_path_field]
                ) == len(row[text_field])
                for text, img_path in zip(row[text_field], row[img_path_field]):
        ...
        paired_data = [
            {"text": text, "img_path": img_path} for text, img_path in unique_pairs
        ]
        return paired_data
...
class FlickrDataset(Dataset):
    def __init__(self, modality, model_backbone):
        ...
        self.raw_data = load_dataset(
            "nlphuji/flickr_1k_test_image_text_retrieval", split="test"
        )
        if modality == "image":
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        if self.ordered_datasets:
            ...
            for n in self.ds_lens:
                assert n % self.chunk_size == 0, (
                    f"Dataset length {n} is not divisible by chunk_size"
                )
                chunks = torch.arange(n).view(
                    -1, self.chunk_size
                )  # shape (n // 16, 16)
            ...
            ds_indices = [
                [i + sum(self.ds_lens[:j]) for i in ds_indices[j]]
                for j in range(len(self.ds_lens))
            ]
            ds_batches = [
                list(torch.split(torch.tensor(ds_indices[j]), self.total_batch_size))
                for j in range(len(self.ds_lens))
            ]
        elif self.same_datasets:
            ds_indices = [
                torch.randperm(n, generator=generator).tolist() for n in self.ds_lens
            ]
            ds_indices = [
                [i + sum(self.ds_lens[:j]) for i in ds_indices[j]]
                for j in range(len(self.ds_lens))
        ...
        if False:
            ...
            order = torch.randperm(
                len(incomplete_indices), generator=generator
            ).tolist()
            ...
            mixed_batches = list(
                torch.split(torch.tensor(incomplete_indices), self.total_batch_size)
            )
            ...
            logger.info(
                f"Using global batch size {self.total_batch_size} created {len(ds_batches) - len(mixed_batches)} single-dataset batches & {len(mixed_batches)} mixed dataset batches."
            )
        else:
            ds_batches = sum(ds_batches, [])
            logger.info(
                f"Using global batch size {self.total_batch_size} created {len(ds_batches)} single-dataset batches."
            )
        ...
        for _ in range(
            self.num_repeats
        ):  # Iterate K times to include the first epoch as well
            epoch_order = torch.randperm(len(ds_batches), generator=generator).tolist()
            shuffled_batches = [
                ds_batches[i] for i in epoch_order
