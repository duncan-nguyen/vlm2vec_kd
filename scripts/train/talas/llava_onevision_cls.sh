#!/bin/bash
# TALAS -- LLaVA-OneVision-0.5B, classification
...
#
#   VARIANT=talas SEED=42 bash scripts/train/talas/llava_onevision_cls.sh
#   for v in talas no_asam sam no_lasd no_tamd; do
#     for s in 42 43 44; do VARIANT=$v SEED=$s bash scripts/train/talas/llava_onevision_cls.sh; done
#   done
...
#   MMEB_TRAIN_DIR=/some/path bash scripts/train/talas/llava_onevision_cls.sh
...
#   TEACHER_CACHE=cache/b3_qwen2_2b_llava_onevision_cls bash scripts/train/talas/llava_onevision_cls.sh
#
...
OUTPUT_DIR="${OUTPUT_DIR:-training/TALAS/llava_onevision_cls/${VARIANT}_seed${SEED}}"
...
# test:  bash scripts/train/talas/llava_onevision_cls.sh --percent_data 0.01
