...
#          other methods; see scripts/train/ours/README.md and
#          docs/ours_implementation.md.
#
# One script, three rows: the method, its no-teacher control and its
# L_topo-only ablation.
# Pick one with VARIANT and vary SEED:
#
#   VARIANT=ours SEED=42 bash scripts/train/ours/llava_onevision_vqa.sh
#   for v in ours student_only; do
#     for s in 42 43 44; do VARIANT=$v SEED=$s bash scripts/train/ours/llava_onevision_vqa.sh; done
#   done
...
#   MMEB_TRAIN_DIR=/duong/dan/khac bash scripts/train/ours/llava_onevision_vqa.sh
...
#   TEACHER_CACHE=cache/b3_qwen2_2b_llava_onevision_vqa bash scripts/train/ours/llava_onevision_vqa.sh
#
...
OUTPUT_DIR="${OUTPUT_DIR:-training/ours/llava_onevision_vqa/${VARIANT}_seed${SEED}}"
...
# test:  bash scripts/train/ours/llava_onevision_vqa.sh --percent_data 0.01 \
