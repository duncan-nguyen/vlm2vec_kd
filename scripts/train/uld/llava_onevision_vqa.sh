#!/bin/bash
# Universal logit distillation -- LLaVA-OneVision-0.5B, VQA
# Zero-pads the narrower embedding space and matches the two with MSE.
#
# Method:  kd_loss_type universal_logit
...
#   MMEB_TRAIN_DIR=/duong/dan/khac bash scripts/train/uld/llava_onevision_vqa.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-training/uld_llava_onevision_vqa}"
SEED="${SEED:-42}"
...
#   TEACHER_CACHE=cache/b3_qwen2_2b_llava_onevision_vqa bash scripts/train/uld/llava_onevision_vqa.sh
#
...
# one flag away:  bash scripts/train/uld/llava_onevision_vqa.sh --percent_data 0.01
