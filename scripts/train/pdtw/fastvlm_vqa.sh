#!/bin/bash
# Proposal + soft-DTW -- FastVLM-0.5B, VQA
...
# ! (src/criterions/proposal_loss_with_DTW.py). FastVLM's input_ids hold a single -200
...
#   MMEB_TRAIN_DIR=/duong/dan/khac bash scripts/train/pdtw/fastvlm_vqa.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-training/pdtw_fastvlm_vqa}"
SEED="${SEED:-42}"
...
# one flag away:  bash scripts/train/pdtw/fastvlm_vqa.sh --percent_data 0.01
