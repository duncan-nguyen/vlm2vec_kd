#!/bin/bash
# CMTop -- LLaVA-OneVision-0.5B, VQA
...
#
#   VARIANT=cmtop_h0 SEED=42 bash scripts/train/cmtop/llava_onevision_vqa.sh
#   for v in student_only endpoint vsp pointcloud_h0 cmtop_h0 cmtop_h0_h1; do
#     for s in 42 43 44; do VARIANT=$v SEED=$s bash scripts/train/cmtop/llava_onevision_vqa.sh; done
#   done
...
#   MMEB_TRAIN_DIR=/duong/dan/khac bash scripts/train/cmtop/llava_onevision_vqa.sh
...
#   TEACHER_CACHE=cache/b3_qwen2_2b_llava_onevision_vqa bash scripts/train/cmtop/llava_onevision_vqa.sh
#
...
OUTPUT_DIR="${OUTPUT_DIR:-training/CMTop/llava_onevision_vqa/${VARIANT}_seed${SEED}}"
...
# test:  bash scripts/train/cmtop/llava_onevision_vqa.sh --percent_data 0.01 \
