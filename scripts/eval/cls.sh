#!/bin/bash
# MMEB evaluation on the CLS benchmarks of Table 1, Precision@1.
#
#   bash scripts/eval/cls.sh <checkpoint-dir>
#
...
CKPT="${1:-$CKPT}"
if [ -z "$CKPT" ]; then
    echo "usage: bash scripts/eval/cls.sh <checkpoint-dir>" >&2
    echo "   or: CKPT=<dir> BACKBONE=llava_onevision IMAGE_RESOLUTION=336 bash scripts/eval/cls.sh" >&2
    exit 1
fi

BACKBONE="${BACKBONE:-llava_qwen2}"
IMAGE_RESOLUTION="${IMAGE_RESOLUTION:-448}"
OUT="${OUT:-$CKPT/mmeb_cls}"

# Nơi chứa ảnh MMEB-eval. Ghi đè: MMEB_EVAL_DIR=/duong/dan/khac bash scripts/eval/cls.sh
